...
async def get_all_data_by_section() -> list[LLMResponse]:
    ...
    for section_code in ["1.7", "2.5", "3.1.1.2", "3.1.2", "3.1.3", "3.1.4"]:
        with open(f"texts/section_names/{section_code}.txt", 'r', encoding='utf-8') as f:
            section_name = f.read()
...
Делай выводы исходя из следующих данных:\n"""
        ...
        with open(f"texts/tables/{section_code}.txt", 'r', encoding='utf-8') as f:
            tables_data = f.read()
            promt += tables_data
        
        with open(f"texts/gold/2024/{section_code}.txt", 'r', encoding='utf-8') as f:
            section_text = f.read()
            promt += f"""
